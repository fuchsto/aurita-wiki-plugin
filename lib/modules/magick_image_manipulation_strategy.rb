...
require('aurita')
require('RMagick') if Aurita::App_Configuration.use_magick
...
module Wiki
  ...
  #
  class Magick_Image_Manipulation_Strategy < Image_Manipulation_Strategy
  include Media_Asset_Helpers
  include Magick if Aurita::App_Configuration.use_magick

    @@logger = Aurita::Log::Class_Logger.new(self)

    def initialize(media_asset_instance)
      @manip_strategy_klass = Magick::ImageList
      super()
    end
