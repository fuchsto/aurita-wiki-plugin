...
module Wiki
  ...
  #
  class Image_Manipulation_Strategy
  ...
  protected
    ...
    def open_image
      ...
      @img = @manip_strategy_klass.new(@img_work_path)
      @img_opened = true
    end
  ...
  public
    ...
    # {{{
      raise ::Exception.new("#{self.class}#image_info has not been defined.")
    end # }}}
...
#     write_work_version()
    ...
    # Every image needs a jpeg base image that is e.g. later used 
    # as a basis for generating thumbnail variants. 
    # In case an image file is not a jpeg, this method generates 
    # an asset_<media_asset_id>.jpg in the projects public/assets 
    # folder. 
    def create_full_preview
      ...
      begin
        img = @manip_strategy_klass.new(path) 
      rescue ::Exception => e
    ...
    # Generate all given image variants. 
    # Example: 
    #
    #   manip.create_image_varians( :circle => Proc.new { |magick_image, media_asset|
    #     # ... generate a circle variant of the image and write it to a file
    #   })
    #
    # Note that when using a different image_manipulation strategy, the 
    # block argument and thus the variant generation code in the Proc object 
      ...
      variants.each_pair { |variant_name, procedure|
        begin
          ...
          if (File.exists?(image_in) && !(File.exists?(image_out))) then
            ...
            img = @manip_strategy_klass.new(image_in)
