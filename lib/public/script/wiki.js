...
*/
  ...
  init_article : function(article_id, params)
  {
    if(params && params.editable) { 
      Sortable.create('article_body_'+article_id, 
                      { tag: 'div', 
                        only: 'article_partial', 
                        handle: 'sort_handle', 
  ...
  media_asset_selection_list_onclick : function(params)
  {
    Aurita.load_widget('Wiki::Media_Asset_Selection_List_Entry', 
                       { media_asset_id: params.media_asset_id, 
                         label: params.label, 
                         name: params.name 
                       }, 
                       function(widget_response) { 
                         widget = Aurita.eval_response(widget_response); 
                         if(!widget && widget['html']) { return; } 
                         widget = widget['html']; 
                         $(params.selection_list_id).innerHTML += widget; 
                       });
  },
  ...
  insert_link : function(article_id_element, url_element) { 
    ...
    tinyMCE.activeEditor.selection.moveToBookmark(Aurita.Wiki.editor_selection_bookmark); // For IE, as it loses focus
    ...
    if($(article_id_element)) { 
      ...
      var new_c = '<a onclick="Aurita.load({ action: \''+link_url+'\' }); return false; " href="#'+link_url+'">'+old_c+'</a>';
...
//  var type = params['type'];
    ...
    if(variant == 'original' || variant == 'none') { 
      elm = '<img src="/aurita/images/assets/asset_'+media_asset_id+'.jpg" />';
    } 
    else { 
  ...
  editor_selection_bookmark : false, 
  ...
  link_to_file : function(media_asset_id) { 
    ...
    old_c = tinyMCE.activeEditor.selection.getContent(); 
    
    elm = '<a href="#'+link_url+'" onclick="Aurita.load({ action: \''+link_url+'\' });">'+old_c+'</a>'
    tinyMCE.activeEditor.selection.setContent(elm); 
  }, 

  link_to_file_download : function(media_asset_id) { 
    var link_url = '/aurita/Wiki::Media_Asset/proxy/media_asset_id='+media_asset_id; 
    var old_c;
    ...
    elm = '<a href="'+link_url+'" target="_self">'+old_c+'</a>'
    tinyMCE.activeEditor.selection.setContent(elm); 
  }, 

  on_media_assets_reorder: function(container) { 
    position_values = Sortable.serialize(container.id);
    Aurita.load_silently({ element: 'dispatcher', 
                           method: 'POST', 
                           action: 'Wiki::Media_Asset/perform_reorder/' + position_values }); 
  }, 

  send_article : function(article_id) {
    $('popbox_container').hide(); 
    $('popbox_container').innerHTML = '';
//  Aurita.GUI.center_element_y('popbox_container');
    Aurita.load({ action: 'Wiki::Article/send/article_id='+article_id, 
                  element: 'popbox_container', 
                  onload: function() { 
                    $('popbox_container').show(); 
                  }
                });
  }, 

  request_file : function(checkbox_elem, thumb_elem, media_asset_id) { 
    if(checkbox_elem.checked) { 
      thumb_elem.addClassName('requested');
    }
    else {
      thumb_elem.removeClassName('requested');
    }
    Aurita.load_silently({ element: 'dispatcher', 
                           method: 'POST', 
                           action: 'Wiki::Media_Asset_Request/toggle/id='+media_asset_id })
  }, 

  approve_file : function(checkbox_elem, thumb_elem, media_asset_id) { 
    if(checkbox_elem.checked) { 
      thumb_elem.addClassName('approved');
    }
    else {
      thumb_elem.removeClassName('approved');
    }
    Aurita.load_silently({ element: 'dispatcher', 
                           method: 'POST', 
                           action: 'Wiki::Media_Asset_Approval/toggle/id='+media_asset_id })
  }
